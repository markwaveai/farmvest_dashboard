            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div className="bg-white p-4 rounded-xl shadow-md border border-gray-100 flex items-center justify-between">
                    <div>
                        ...
                        <h3 className="text-lg font-bold text-red-600 mt-1">06 Items</h3>
                    </div>
                ...
                <div className="bg-white p-4 rounded-xl shadow-md border border-gray-100 flex items-center justify-between">
                    <div>
                        ...
                        <h3 className="text-lg font-bold text-emerald-700 mt-1">12 Items</h3>
                    </div>
                                    ...
                                    <tr key={item.id} className="hover:bg-gray-50 transition-colors text-sm">
                                        <td className="px-4 py-3 whitespace-nowrap">
                                            <div className="flex items-center gap-3">
                                                <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${item.name.toLowerCase().includes('vaccine') ? 'bg-orange-50 text-orange-500' : 'bg-blue-50 text-blue-900'}`}>
                                                    {item.name.toLowerCase().includes('vaccine') ? <Wrench size={14} /> : <Box size={14} />}
                                                </div>
                                                <div>
                                                    <div className="font-semibold text-gray-900 text-sm">{item.name}</div>
                                                    <div className="text-[10px] text-gray-500 font-medium">{item.batch}</div>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-4 py-3 whitespace-nowrap align-middle">
                                            <div className="w-full max-w-[140px] mx-auto">
                                                ...
                                                <div className="h-1.5 w-full bg-gray-100 rounded-full overflow-hidden">
                                                    <div className={`h-full rounded-full transition-all duration-500 ${item.status === 'critical' ? 'bg-red-500' : item.status === 'warning' ? 'bg-orange-500' : 'bg-emerald-500'}`} style={{ width: `${(item.remaining / item.total) * 100}%` }}></div>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-4 py-3 whitespace-nowrap">
                                        ...
                                        <td className="px-4 py-3 whitespace-nowrap">
                                            <div className="flex items-center gap-2">
                                                <div className={`w-2 h-2 rounded-full ${item.vet.includes('Sarah') ? 'bg-orange-400' : 'bg-teal-400'}`}></div>
                                                <span className="text-gray-700 font-medium">{item.vet}</span>
                                            </div>
                                        </td>
                                        <td className="px-4 py-3 whitespace-nowrap text-right">
                                            <button className="text-blue-900 hover:text-blue-800 font-bold text-xs bg-blue-50 hover:bg-blue-100 px-3 py-1.5 rounded-md transition-colors flex items-center gap-1 ml-auto">
